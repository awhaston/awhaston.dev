...
{#each jobs as job}
	...
	<div class="title" on:click={() => (job.hidden = !job.hidden)}>
		<Arrow classes={job.hidden ? 'inactive' : 'active'} /><span class="date">{job.date}:</span
		>{job.title}
	</div>
	<div class="overflow {job.hidden ? 'hidden' : 'show'}">
		{job.description}
